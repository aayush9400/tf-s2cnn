...
def s2_mm(x, y):
    ...
    for l in range(nl):
        ...
        Fx = tf.reshape(Fx, (L * nbatch, nfeature_in, 2))  # [m * batch, feature_in, complex]

        Fy = tf.transpose(Fy, perm=[1, 0, 2, 3])  # [feature_in, m, feature_out, complex]
        Fy = tf.reshape(Fy, (nfeature_in, L * nfeature_out, 2))  # [feature_in, m * feature_out, complex]

        # Assuming complex_mm is a predefined function for complex matrix multiplication
        Fz = complex_mm(Fx, Fy, conj_y=True)  # [m_x * batch, m_y * feature_out, complex] m_x -> m, m_y -> n
        Fz = tf.reshape(Fz, (L, nbatch, L, nfeature_out, 2))  # [m, batch, n, feature_out, complex]

        # When using tf.transpose, the `perm` parameter is used to permute the dimensions
        Fz = tf.transpose(Fz, perm=[0, 2, 1, 3, 4])  # [m, n, batch, feature_out, complex]
        Fz = tf.reshape(Fz, (L * L, nbatch, nfeature_out, 2))  # [m * n, batch, feature_out, complex]

        Fz_list.append(Fz)

        begin += size

    z = tf.concat(Fz_list, axis=0)  # [l * m * n, batch, feature_out, complex]
    return z
...
#     '''
#     import s2cnn.utils.cuda as cuda_utils
#     assert x.is_cuda and x.dtype == tf.float32
#     assert y.is_cuda and y.dtype == tf.float32
#     assert y.size(3) == 2
#     assert x.size(3) == 2
#     nbatch = x.size(1)
#     nfeature_in = x.size(2)
...
def test_compare_cuda_cpu():
    x = tf.rand(1+3+5+7, 2, 3, 2)  # [l * m,     batch,      feature_in,  complex]
    y = tf.rand(1+3+5+7, 3, 5, 2)  # [l * m,     feature_in, feature_out, complex]
    z1 = s2_mm(x, y)
