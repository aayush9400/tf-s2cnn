...
def so3_mm(x, y):
    ...
    for l in range(nl):
        ...
        Fx = tf.reshape(Fx, (L, L, nbatch, nfeature_in, 2))  # [m, n, batch, feature_in, complex]
        Fx = tf.transpose(Fx, perm=[1, 0, 2, 3, 4])  # [n, m, batch, feature_in, complex]
        Fx = tf.transpose(Fx, perm=[2, 1, 0, 3, 4])  # [batch, m, n, feature_in, complex]
        Fx = tf.transpose(Fx, perm=[0, 1, 3, 2, 4])  # [batch, m, feature_in, n, complex]
        Fx = tf.reshape(Fx, (nbatch * L, nfeature_in * L, 2))  # [batch * m, feature_in * n, complex]

        Fy = tf.reshape(Fy, (L, L, nfeature_in, nfeature_out, 2))  # [m, n, feature_in, feature_out, complex]
        Fy = tf.transpose(Fy, perm=[2, 1, 0, 3, 4])  # [feature_in, n, m, feature_out, complex]
        Fy = tf.reshape(Fy, (nfeature_in * L, L * nfeature_out, 2))  # [feature_in * n, m * feature_out, complex]

        Fz = complex_mm(Fx, Fy, conj_y=True)  # replace with the correct TensorFlow complex multiplication method
        Fz = tf.reshape(Fz, (nbatch, L * L, nfeature_out, 2))  # [batch, m * n, feature_out, complex]
        Fz = tf.transpose(Fz, perm=[1, 0, 2, 3])  # [m * n, batch, feature_out, complex]

        Fz_list.append(Fz)

        begin += size

    z = tf.concat(Fz_list, 0)  # [l * m * n, batch, feature_out, complex]
    return z
...
def test_compare_cuda_cpu():
    x = tf.rand(1+9+25+49, 2, 3, 2)  # [l * m * n, batch,      feature_in,  complex]
    y = tf.rand(1+9+25+49, 3, 5, 2)  # [l * m * n, feature_in, feature_out, complex]
    z1 = so3_mm(x, y)
